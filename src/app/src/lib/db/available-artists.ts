...
export const getAvailableArtistsForBooking = async (
  ...
  const distinctServiceIds = [...new Set(serviceIds.filter(Boolean))];
  ...
  try {
    ...
    if (error) {
      dbLogger.error("Failed to fetch available artists", { error });
      throw error;
    }

    return (data ?? []) as AvailableArtistForBooking[];
  } catch (error) {
    dbLogger.error("Error in getAvailableArtistsForBooking", { error });
    throw error;
  }
};

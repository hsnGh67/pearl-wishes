...
export function Features() {
  ...
  return (
    <section id="features" className="py-12 md:py-16 lg:py-20 px-4 md:px-6 lg:px-8" style={{ backgroundColor: '#FAF7F5' }}>
      <div className="max-w-7xl mx-auto">
        {/* Section Header */}
        <div className="text-left md:text-center mb-8 md:mb-12 lg:mb-16">
          <h2 className="mb-3 md:mb-4 font-semibold" style={{ color: '#3D3935' }}>Our Treatments</h2>
          <p className="max-w-2xl mx-auto px-4" style={{ color: '#3D3935' }}>
            Discover our range of premium nail care services designed to make you look and feel your best
          </p>
        </div>
        ...
        <div className="flex md:grid overflow-x-auto md:overflow-x-visible gap-4 md:gap-6 lg:gap-8 md:grid-cols-2 lg:grid-cols-3 snap-x snap-mandatory md:snap-none px-4 md:px-0">
          {services.map((service, index) => (
            <Card 
              ...
              style={{ backgroundColor: '#FEFCFA', border: '1px solid #DCD4CD' }}
            >
              ...
              <CardContent className="p-5 md:p-6 flex flex-col flex-1">
                {/* Title */}
                <h3 className="mb-3 text-lg md:text-xl" style={{ color: '#3D3935' }}>
                  {service.title}
                </h3>
                
                {/* Description */}
                <p className="mb-5 flex-grow text-sm md:text-base leading-relaxed" style={{ color: '#3D3935' }}>
                  {service.description}
                </p>
